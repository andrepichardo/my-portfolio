...
export const SECTION_DEFAULTS: Record<SectionKey, SectionContent> = {
  ...
  about: {
    ...
    body: [
      ...
      "Outside the office I take on client work — WordPress and e-commerce sites for local businesses — but the project that keeps me busiest is Propio, my own SaaS for independent landlords. I have built it entirely on my own: the product design, the interface, the API, the database and the deployment. It keeps properties, tenants and contracts in one place, reconciles rent payments, issues PDF receipts and monthly statements, and turns all of it into reports an owner can actually read. It is the one place where I make every decision and then live with all of them.",
